...
func LatLngToTile(lat, lng float64, zoom int) (int, int) {
	...
	if y < 0 { y = 0 }
	if y >= int(n) { y = int(n) - 1 }
	if x < 0 { x = 0 }
	if x >= int(n) { x = int(n) - 1 }
	return x, y
}

// BoundingBoxToTiles returns all tile coordinates in a bounding box at a zoom level
func BoundingBoxToTiles(minLat, minLng, maxLat, maxLng float64, zoom int) [][3]int {
	...
	for z := zoom; z >= zoom; z-- {
		for yy := y1; yy <= y2; yy++ {
			for xx := x1; xx <= x2; xx++ {
				tiles = append(tiles, [3]int{z, xx, yy})
			}
		}
	}
	return tiles
